...
class Analyzer {
    ...
    analyze({ document, uri, // NOTE: we don't use document.uri to make testing easier
        ...
        function findMissingNodes(node) {
            if (node.isMissing()) {
                diagnostics.push(LSP.Diagnostic.create(TreeSitterUtil.range(node), `Syntax error: expected "${node.type}" somewhere in the file`, LSP.DiagnosticSeverity.Warning));
            }
    ...
    findReferences(word) {
        ...
        return (0, flatten_1.flattenArray)(uris.map((uri) => this.findOccurrences(uri, word)));
    }
    /**
