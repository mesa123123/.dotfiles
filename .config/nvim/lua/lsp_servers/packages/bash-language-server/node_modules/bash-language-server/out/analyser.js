 ...
 */
class Analyzer {
    constructor({ includeAllWorkspaceSymbols = false, parser, workspaceFolder, }) {
    ...
    analyze({ document, uri, // NOTE: we don't use document.uri to make testing easier
        ...
        const { diagnostics, globalDeclarations } = (0, declarations_1.getGlobalDeclarations)({ tree, uri });
        ...
        function findMissingNodes(node) {
            if (node.isMissing()) {
                diagnostics.push(LSP.Diagnostic.create(TreeSitterUtil.range(node), `Syntax error: expected "${node.type}" somewhere in the file`, LSP.DiagnosticSeverity.Warning, undefined, 'bash-language-server'));
            }
        ...
        return uris.filter((uri) => {
            if (!this.uriToAnalyzedDocument[uri]) {
                ...
                try {
                    ...
                    logger_1.logger.warn(`Error while analyzing sourced file ${uri}: ${err}`);
                    return false;
                }
            }
            return true;
